...
main (int argc, char *argv[])
{
  ...
  //enbPositionAlloc->Add (Vector (0.0, 1000.0, 0.0));                       // eNB1
  ...
  edgeUePositionAlloc->Add (Vector (distance * 0.5, 0, 0.0));  // edgeUE1
  edgeUePositionAlloc->Add (Vector (distance * 0.5, 0, 0.0));  // edgeUE1
  ...
  Simulator::Destroy ();
    for (int j = 0; j < 2; ++j) {
        uint64_t sum = DynamicCast<PacketSink> (serverApps.Get(j))->GetTotalRx ();
        double goodput = sum * 8 / (simTime - .5);
        cout << "Node " << j << " packets: " << sum << " Goodput: " << goodput << "\n";
    }
  return 0;
}